...
from py_librenms.models.device_groups import DeviceGroupDevicesResponse, DeviceGroupsResponse
from py_librenms.routes.device_groups import DeviceGroups
...
class TestAddDevicegroup:
    def test_posts_payload(self, mock_client) -> None:
        ...
        route = DeviceGroups(mock_client)
        result = asyncio.run(route.add_devicegroup("Core", "static", devices=[1, 2]))
        mock_client._post.assert_called_once_with(
            "/devicegroups", json={"name": "Core", "type": "static", "devices": [1, 2]}
        )
...
class TestGetDevicesByGroup:
    def test_calls_correct_path(self, mock_client) -> None:
        ...
        mock_client._get.assert_called_once_with("/devicegroups/Core", params={})
    ...
    def test_with_full_flag(self, mock_client) -> None:
        ...
        mock_client._get.assert_called_once_with("/devicegroups/Core", params={"full": 1})
