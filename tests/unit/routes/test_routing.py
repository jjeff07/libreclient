...
class TestListBgp:
    def test_no_params(self, mock_client) -> None:
        mock_client._get.return_value = {"status": "ok", "message": "", "count": 0, "routing": []}
    ...
    def test_with_hostname(self, mock_client) -> None:
        ...
        mock_client._get.assert_called_once_with("/bgp", params={"hostname": "router1"})


class TestGetBgp:
    def test_calls_correct_path(self, mock_client) -> None:
        mock_client._get.return_value = {"status": "ok", "message": "", "count": 1, "routing": []}
        route = Routing(mock_client)
...
class TestEditBgpDescr:
    def test_posts_description(self, mock_client) -> None:
        ...
        mock_client._post.assert_called_once_with("/bgp/3", json={"bgp_descr": "Transit peer"})
...
class TestListVrf:
    ...
    def test_with_hostname(self, mock_client) -> None:
        ...
        mock_client._get.assert_called_once_with("/routing/vrf", params={"hostname": "r1"})

    def test_with_vrfname(self, mock_client) -> None:
        mock_client._get.return_value = {"status": "ok", "message": "", "count": 0, "routing": []}
        route = Routing(mock_client)
        asyncio.run(route.list_vrf(vrfname="MGMT"))
        mock_client._get.assert_called_once_with("/routing/vrf", params={"vrfname": "MGMT"})

    def test_raises_if_both_set(self, mock_client) -> None:
        route = Routing(mock_client)
        with pytest.raises(ValueError, match="hostname and vrfname cannot both be set"):
...
class TestListIpAddresses:
    ...
    def test_ipv4_only(self, mock_client) -> None:
        ...
        mock_client._get.assert_called_once_with("/resources/ip/addresses/ipv4")
