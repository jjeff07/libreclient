...
class TestGetDevice:
    def test_calls_correct_path(self, mock_client) -> None:
        mock_client._get.return_value = {"status": "ok", "message": "", "count": 1, "devices": []}
...
class TestListDevices:
    def test_no_params(self, mock_client) -> None:
        mock_client._get.return_value = {"status": "ok", "message": "", "count": 0, "devices": []}
        route = Devices(mock_client)
    ...
    def test_with_type_filter(self, mock_client) -> None:
        ...
        mock_client._get.assert_called_once_with("/devices", params={"type": "active"})
...
class TestAddDevice:
    def test_posts_payload(self, mock_client) -> None:
        ...
        route = Devices(mock_client)
        result = asyncio.run(route.add_device("newhost", snmpver="v2c", community="public"))
        mock_client._post.assert_called_once_with(
            "/devices", json={"hostname": "newhost", "snmpver": "v2c", "community": "public"}
        )
        assert isinstance(result, ApiResponse)
...
class TestGetDevicePorts:
    def test_calls_correct_path(self, mock_client) -> None:
        mock_client._get.return_value = {"status": "ok", "message": "", "count": 0, "ports": []}
        route = Devices(mock_client)
        result = asyncio.run(route.get_device_ports("sw1"))
        mock_client._get.assert_called_once_with("/devices/sw1/ports", params={})
...
class TestGetComponents:
    def test_with_type_filter(self, mock_client) -> None:
        ...
        mock_client._get.assert_called_once_with("/devices/sw1/components", params={"type": "fan"})
...
class TestGetHealthGraph:
    def test_returns_bytes(self, mock_client) -> None:
        ...
        mock_client._get_bytes.assert_called_once_with("/devices/sw1/graphs/health/temperature")
        assert isinstance(result, bytes)


class TestUpdateDeviceField:
    def test_patches_field(self, mock_client) -> None:
        mock_client._patch.return_value = {"status": "ok", "message": "updated"}
        route = Devices(mock_client)
        result = asyncio.run(route.update_device_field("sw1", "notes", "test note"))
...
class TestRenameDevice:
    def test_patches_rename(self, mock_client) -> None:
        mock_client._patch.return_value = {"status": "ok", "message": "renamed"}
        route = Devices(mock_client)
        result = asyncio.run(route.rename_device("old-name", "new-name"))
        mock_client._patch.assert_called_once_with("/devices/old-name/rename/new-name")
        assert isinstance(result, ApiResponse)
