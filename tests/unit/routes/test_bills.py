...
class TestListBills:
    def test_no_params(self, mock_client) -> None:
        mock_client._get.return_value = {"status": "ok", "message": "", "count": 0, "bills": []}
    ...
    def test_with_period(self, mock_client) -> None:
        ...
        mock_client._get.assert_called_once_with("/bills", params={"period": "previous"})


class TestGetBill:
    def test_by_id(self, mock_client) -> None:
        mock_client._get.return_value = {"status": "ok", "message": "", "count": 1, "bills": []}
        route = Bills(mock_client)
    ...
    def test_by_ref(self, mock_client) -> None:
        ...
        mock_client._get.assert_called_once_with("/bills", params={"ref": "REF123"})
...
class TestGetBillGraph:
    def test_returns_bytes(self, mock_client) -> None:
        ...
        mock_client._get_bytes.assert_called_once_with("/bills/1/graphs/day", params={})
...
class TestCreateEditBill:
    def test_posts_kwargs(self, mock_client) -> None:
        ...
        route = Bills(mock_client)
        result = asyncio.run(route.create_edit_bill(bill_name="Transit", bill_type="cdr"))
