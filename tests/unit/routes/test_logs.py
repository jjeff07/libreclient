...
class TestListEventlog:
    def test_no_params(self, mock_client) -> None:
        mock_client._get.return_value = {"status": "ok", "message": "", "count": 0, "logs": []}
    ...
    def test_with_hostname(self, mock_client) -> None:
        ...
        mock_client._get.assert_called_once_with("/logs/eventlog/sw1", params={"limit": 10})
...
class TestSyslogsink:
    def test_posts_messages(self, mock_client) -> None:
        mock_client._post.return_value = {"status": "ok", "message": "accepted"}
        route = Logs(mock_client)
        result = asyncio.run(route.syslogsink([{"msg": "test"}]))
        mock_client._post.assert_called_once_with("/syslogsink", json=[{"msg": "test"}])
    ...
    def test_wraps_single_dict(self, mock_client) -> None:
        ...
        mock_client._post.assert_called_once_with("/syslogsink", json=[{"msg": "single"}])
...
class TestAddEventlog:
    def test_posts_payload(self, mock_client) -> None:
        ...
        mock_client._post.assert_called_once_with(
            "/devices/sw1/eventlog", json={"text": "rebooted", "severity": "warning"}
        )
        assert isinstance(result, ApiResponse)
